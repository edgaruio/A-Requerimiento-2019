...
dashboardPage(skin = "blue",
              dashboardHeader(title = "Calle 72 y 100"),
              dashboardSidebar(
                sidebarMenu(disable = TRUE, br(),
                            ...
                            menuItem("EMPRESAS", tabName = "empresas", icon = icon("dashboard"),
                                     ...
                                     selectInput("xpoligono", label = "Cuadrante:",
                                                 choices = c("TOTAL","Calle 72 a 83"="calle72_83","Calle 83 a 92"="calle83_92","Calle 92 a 100"="calle92_100"),
                ...
                tabItems(
                  tabItem("dummy",
                          fluidRow(
                            ...
                            column(10,
                                   ...
                                   br(),
                                   h3("Nota: Debe dar click en el boton 'Aplicar Filtros' para cambiar el panel principal. 
                                      Al cambiar los filtros debe dar click en 'Go' para visualizar la información"),
                                   h4("Fecha de actualizacion: 12/03/2020 (Corte Febrero)")
                            ),
                            column(1)
                          )
                  ),
                  tabItem(tabName = "empresas",
                          ...
                          fluidRow(
                            column(width = 6,
                                 fluidRow(
                                   box(title = "Mapa Empresas",status = "primary", solidHeader = TRUE,collapsible = TRUE,
                                       withLoader(leafletOutput("Mapaempresas", height = 750), type = "html", loader = "loader1"), width=12)
                                   ),
                                 fluidRow(
                  ...
                  tabItem(tabName = "personas",
                          ...
                          fluidRow(
                            column(width = 6,
                                   fluidRow(
                                     box(title = "Mapa Personas (Viven y/o Trabajan)",status = "primary", solidHeader = TRUE,collapsible = TRUE,
                                         withLoader(leafletOutput("Mapaafiliados", height = 750), type = "html", loader = "loader1"), width=12)
                                     # downloadButton("downloadData2", "Descargar Base Afiliados")
                                   )
                            ),
                            column(width = 6,
                                   fluidRow(
