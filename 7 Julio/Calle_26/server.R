...
#### PERSONAS ===============================
  ...
  output$Mapaafiliados <- renderLeaflet({
    ...
    map %>%
      ...
      addPolygons(data=poligonos, color = "teal", opacity = 0.1) %>% 
      setView(-74.111902, 4.656071, zoom = 14)
  })
  ...
  output$resumen1 <- renderPlotly({
    ...
    aux1 %>%
      plot_ly(x= ~pop, y=~edad_agru,color=~Genero) %>%
      add_bars(orientation = 'h', hoverinfo = 'text', text = ~paste(comma(abs_pop), "(", round(freq*100,2), "%)"), 
               textposition = "outside", textfont = list(color = 'darkgrey', size = 10)) %>%
      layout(margin = m,
             bargap = 0.1,
             barmode = 'overlay',
             title = 'Piramide Poblacional', 
